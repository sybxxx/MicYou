...
export function useServer(options?: {
	...
	const connectionMode = useStorage<ConnectionMode>(
		"micyou_connectionMode",
		"wifi",
	);
	...
	const outputDevice = ref<string>(
		localStorage.getItem("micyou_output_device") || "",
	);
	...
	const statusDescription = computed(() => {
		...
		const port =
			activePort.value || (mode === "web" ? webPort.value : serverPort.value);
	...
	const toggleStreaming = async () => {
		if (serverState.value !== "idle") {
			try {
				...
				if (isMacOS) {
					try {
						await invoke("restore_input_device");
					} catch {
			...
			try {
				...
				activePort.value =
					connectionMode.value === "web"
						? Number(webPort.value)
						: Number(serverPort.value);
				...
				if (isMacOS) {
					try {
						await invoke("set_blackhole_as_input");
					} catch {
						/* best-effort cleanup, ignore */
					}
				}
				if (activeConnectionMode.value === "usb") {
					const result = await invoke<{ type: string; devices?: AdbDevice[] }>(
						"enable_usb_mode",
						{ port: activePort.value, deviceSerial: null },
					);
					if (result.type === "MultipleDevices") {
						...
						activePort.value = null;
						const msg =
							"No USB devices found. Please connect a device and enable USB debugging.";
						const type = analyzeError(msg);
						errorDetails.value = generateErrorDetails(
							type,
							msg,
							activeConnectionMode.value || connectionMode.value,
							activePort.value || Number(serverPort.value),
							selectedIp.value,
							t,
						);
		...
		if (
			...
			try {
				try {
					...
					console.warn("stop_server during IP switch:", e);
				}
				serverState.value = "idle";
				activeConnectionMode.value = null;
				activePort.value = null;
	...
	onMounted(async () => {
		...
		try {
			networkInterfaces.value = await invoke<NetworkInterfaceInfo[]>(
		...
		unlistenDeviceDisconnected = await listen(
			"device-disconnected",
			async () => {
				if (serverState.value === "streaming") {
					const mode = activeConnectionMode.value || connectionMode.value;
